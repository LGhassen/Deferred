...
inline float GetConeMipLevel(float hitDistance, float smoothness, out float sizeInPixels)
{
    ...
    sizeInPixels = 0.75 * rightTriangleOppositeSide * max(SSRScreenResolution.x, SSRScreenResolution.y); // 0.75  is a fudge  factor to match the
                                                      ...
                                                      float smoothness, float dotVN, out float2 ddx, out float2 ddy)
{
    ...
    float sizeInPixels = 0.75 * rightTriangleOppositeSide * max(SSRScreenResolution.x, SSRScreenResolution.y); // 0.75  is a fudge  factor to match the reference importance sampled version
