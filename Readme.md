...
## Transparency
Traditional transparency doesn't work in deferred rendering for opaque objects (only used in the editors in KSP). To emulate transparency, a stylized dissolve effect is used on fairing-only shaders
A dithering effect can be applied on regular shaders but is disabled by default because it is distracting (you'll find an option in the settings file).
